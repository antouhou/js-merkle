...
const trees = maxCase.map((theCase, index) => {
    return maxCase.slice(0, index + 1);
});
...
const casesByTrees = trees.map(treeElements => {
    ...
    return indexCombinations.map((indexCombination) => {
        return new MerkleProofTestCase(leaves, indexCombination, merkleTree.getRoot());
    });
});
...
describe("MerkleProof", () => {
    ...
    after('Autogenerated test summary', () => {
    ...
    describe('#getRoot', () => {
        it('should get a correct root', () => {
            ...
            const binaryRoot = merkleProof.calculateRootWithLeavesCount(leafIndicesToProve, leafHashesToProve, leavesCount);
        ...
        describe('should get a correct root', () => {
            testCases.forEach(testCase => {
                it(testCase.title, () => {
                    ...
                    const treeDepth = merkleTree.getDepth();

                    const root = merkleProof.calculateRootWithLeavesCount(leafIndicesToProve, leafHashesToProve, testCase.leafHashes.length);
    ...
    describe('#verify', () => {
        it('should return true the root matches the expected root', () => {
            ...
            const verified = merkleProof.verify(binaryRoot, leafIndicesToProve, leafHashesToProve, leavesCount);
